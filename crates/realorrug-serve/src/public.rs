...
pub async fn leaderboard() -> Response {
    let paths = Paths::from_env();
    let doc = leaderboard_in(&paths, realorrug_analyst::daemon::now());
...
pub async fn weeks() -> Response {
    let paths = Paths::from_env();
    let doc = weeks_in(&paths, realorrug_analyst::daemon::now());
    respond(&paths, StatusCode::OK, doc)
}
...
pub async fn hunters() -> Response {
    let paths = Paths::from_env();
    let doc = hunters_in(&paths, realorrug_analyst::daemon::now());
    respond(&paths, StatusCode::OK, doc)
}
...
fn rule_doc(rule: Option<&realorrug_contest::Rules>) -> Value {
...
pub fn hunters_in(paths: &Paths, now: u64) -> Value {
    ...
    let board: Vec<realorrug_contest::hunter::Placing> = week
        .and_then(|w| {
            std::fs::read_to_string(realorrug_analyst::contest::hunter_path(
                &paths.contest_dir,
                w,
            ))
            .ok()
...
fn replies(paths: &Paths) -> Vec<Entry> {
    realorrug_analyst::log::latest(&format!("{}/replies.jsonl", paths.analyst_dir))
        .unwrap_or_default()
}
...
fn records(dir: &str) -> Vec<Record> {
    ...
    realorrug_contest::records_in(std::path::Path::new(dir))
}
...
fn reason_name(why: &realorrug_contest::Excluded) -> String {
    use realorrug_contest::Excluded as E;
    match why {
        E::Operator => "operator",
...
mod tests {
    ...
    fn a_summary() -> Summary {
        Summary {
            ...
            creators: 116_752,
            population: realorrug_roast::creator::Population {
                launches: 508_814,
                measured: 506_991,
                organic: 8_999,
                instant: 5_230,
    ...
    fn write_record(paths: &Paths, record: &Record) {
        ...
        realorrug_analyst::contest::write_record(&paths.contest_dir, record).expect("write");
    }
    ...
    fn the_weeks_document_says_why_a_week_paid_nobody_rather_than_leaving_it_blank() {
        ...
        lapsed.claim = None;
        for (n, why) in [
            (1u8, realorrug_contest::Excluded::AccountTooNew { days: 3 }),
            (2, realorrug_contest::Excluded::AccountTooNew { days: 9 }),
            (3, realorrug_contest::Excluded::Operator),
        ] {
    ...
    fn a_voided_week_that_was_already_claimed_still_reads_as_voided() {
        ...
        record.voided = Some(realorrug_contest::ledger::Voided {
    ...
    fn a_paid_week_carries_everything_a_reader_needs_to_check_it_themselves() {
        ...
        let closed = WEEK.closes_at();
        let mut record = a_record(WEEK);
        record.ranking.ranked[0].entry.metrics.verified =
            Some(realorrug_contest::score::Verified {
                reposts: 4,
                quoters: 1,
                likes: 6,
                engagers: 9,
    ...
    fn a_closed_week_publishes_the_evidence_the_rule_was_applied_to() {
        ...
        let week = Week(2957);
        let mut record = a_record(week);
        record.ranking.ranked[0].entry.metrics = realorrug_contest::Metrics {
            reposts: 4,
            quotes: 30,
            likes: 7,
            replies: 2,
            verified: Some(realorrug_contest::Verified {
                reposts: 4,
                quoters: 1,
                likes: 6,
                engagers: 9,
    ...
    fn a_voided_week_says_so_and_says_why_in_the_operators_words() {
        ...
        let week = Week(2957);
        let mut record = a_record(week);
        record.voided = Some(realorrug_contest::Voided {
    ...
    fn the_hunter_board_serves_the_tally_the_week_close_wrote() {
        ...
        let paths = paths_in(dir.path());
        std::fs::create_dir_all(&paths.contest_dir).expect("mkdir");
        realorrug_analyst::contest::write_record(&paths.contest_dir, &a_record(WEEK))
            .expect("write");
        ...
        std::fs::write(
            realorrug_analyst::contest::hunter_path(&paths.contest_dir, WEEK),
    ...
    fn the_paths_default_to_where_the_timers_write_and_the_origin_is_optional() {
        ...
        assert_eq!(none.base_rates, realorrug_roast::baserates::DEFAULT_PATH);
        assert_eq!(none.summary, realorrug_roast::creator::SUMMARY_PATH);
    ...
    fn an_open_week_lists_its_summoners_with_no_score_rather_than_zero() {
        ...
        realorrug_analyst::log::append(&log, &a_reply(now - 60, Some("r1"))).expect("append");
        realorrug_analyst::log::append(&log, &a_reply(now - 30, None)).expect("append");
        // Last week's reply does not belong to this week's page.
        realorrug_analyst::log::append(&log, &a_reply(WEEK.opens_at() - 60, Some("r0")))
            .expect("append");
    ...
    fn exclusions_are_published_as_counts_and_never_as_named_rows() {
        ...
        record.ranking.excluded.push((
            ...
            realorrug_contest::Excluded::AccountTooNew { days: 3 },
        ));
        record.ranking.excluded.push((
            ContestEntry {
            ...
            realorrug_contest::Excluded::AccountTooNew { days: 9 },
        ));
        record.ranking.excluded.push((
            ContestEntry {
            ...
            realorrug_contest::Excluded::Unscored,
        ));
        std::fs::create_dir_all(&paths.contest_dir).expect("mkdir");
        std::fs::write(
            format!("{}/{}.json", paths.contest_dir, WEEK.0),
