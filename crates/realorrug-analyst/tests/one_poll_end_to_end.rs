...
//! which `realorrug-onchain` owns and tests against real captures.

use std::io::{Read as _, Write as _};
use std::time::Duration;

use realorrug_analyst::admission::{Gate, Limits};
use realorrug_analyst::daemon::{Paths, tick};
use realorrug_analyst::publish::DryRun;
use realorrug_analyst::spend::{Prices, Spend};
use realorrug_analyst::x::X;
...
fn funded(paths: &Paths) -> Spend {
    Spend::open(
        ...
        prices(),
        paths.ledger.clone(),
        realorrug_analyst::daemon::day_of(realorrug_analyst::daemon::now()),
    )
}
...
fn no_chain() -> realorrug_onchain::RpcClient {
...
fn one_poll_reads_answers_and_advances_the_cursor() {
    ...
    let cursor = realorrug_analyst::read_cursor(&paths.cursor);
    assert_eq!(
        cursor.as_deref(),
        Some("1003"),
...
impl realorrug_analyst::publish::Publisher for Posts {
    ...
    fn reply(
        ...
        in_reply_to: &str,
        _text: &str,
    ) -> Result<String, realorrug_analyst::publish::Undeliverable> {
        Ok(format!("posted-{in_reply_to}"))
    }
    fn post(&self, _text: &str) -> Result<String, realorrug_analyst::publish::Undeliverable> {
...
fn a_published_reply_is_counted_charged_and_remembered() {
    ...
    let raw = realorrug_analyst::log::read(&paths.log).expect("a log");
...
fn a_platform_that_refuses_costs_nothing_and_does_not_move_the_cursor() {
    ...
    assert_eq!(
        realorrug_analyst::read_cursor(&paths.cursor),
        None,
...
fn tick_against_empty_chain(
    ...
    page: &str,
    limits: Limits,
    publisher: &dyn realorrug_analyst::publish::Publisher,
) -> (Vec<realorrug_analyst::log::Entry>, usize, Option<String>) {
    let (base, _seen) = platform(page);
    ...
    let logged = if std::path::Path::new(&paths.log).exists() {
        realorrug_analyst::log::latest(&paths.log).expect("the log reads")
    } else {
        Vec::new()
    };
    (
...
fn a_thirty_mention_burst_from_one_account_reads_the_chain_at_most_cap_times() {
    ...
    for i in 0..30u8 {
        let mint = realorrug_types::Address::new([i + 1; 32]).to_string();
        data.push(format!(
    ...
    let one = format!(
        ...
        realorrug_types::Address::new([77u8; 32])
    );
...
fn a_telegram_message_is_answered_into_its_own_log_and_never_into_the_record() {
    ...
    let paths = Paths::under(&dir);
    let bot = realorrug_analyst::telegram::Telegram::at(base, "1:token");
    ...
    let logged = realorrug_analyst::log::latest(&paths.telegram_log).expect("the telegram log");
    ...
    assert_eq!(
        realorrug_analyst::read_cursor(&paths.telegram_cursor).as_deref(),
        Some("43"),
...
fn a_telegram_reply_that_is_sent_is_counted_and_remembered_by_the_gate() {
    ...
    let again = realorrug_analyst::telegram::tick(
        Some(&bot),
        &Posts,
        &mut gate,
        &mut spend,
    ...
    assert_eq!(
        realorrug_analyst::log::latest(&paths.telegram_log)
            .expect("log")
            .len(),
        1
    );
...
impl realorrug_model::Provider for Priced {
    fn name(&self) -> &'static str {
        "priced"
    }
...
impl realorrug_analyst::publish::Publisher for Posting {
    fn reply(
        &self,
        to: &str,
        text: &str,
...
fn a_symbol_gets_an_answer_rather_than_silence() {
    ...
    let recorded = realorrug_analyst::log::read(&paths.log).expect("the log");
...
fn a_second_asker_is_pointed_at_the_answer_rather_than_ignored() {
    ...
    let log = realorrug_analyst::log::read(&paths.log).expect("the log");
    let pointer = log
        .iter()
...
fn a_restart_reads_the_days_replies_back_off_disk() {
    ...
    let replies = realorrug_analyst::log::read(&paths.log).expect("the log");
    fresh.restore(&replies, realorrug_analyst::daemon::now());
    assert_eq!(
        fresh.sent_today(),
        1,
...
fn a_week_whose_reads_the_budget_will_not_cover_does_not_close_and_does_not_call() {
    ...
    let week = realorrug_contest::Week(2957);
    realorrug_analyst::log::append(
        &paths.log,
        &realorrug_analyst::log::Entry {
            at: week.opens_at() + 10,
    ...
    let mut spend = Spend::open(
        ...
        prices(),
        paths.ledger.clone(),
        realorrug_analyst::daemon::day_of(week.closes_at() + 5),
    );
    let x = X::at(base, "tok", "u42");
    let rules = realorrug_contest::score::Rules::published(["radar"]);

    let closed = realorrug_analyst::contest::close_if_due(
        Some(&x),
        &paths,
        week.closes_at() + 5,
        &rules,
