...
pub fn hunters(record: &Record, board: &[realorrug_contest::hunter::Placing]) -> Option<Post> {
...
///
/// [`realorrug_roast::render::for_publication`] strips zero-width and bidirectional
...
pub fn checked_text(post: &Post) -> Result<String, Vec<String>> {
    let text = realorrug_roast::render::for_publication(&post.text);
    let mut why: Vec<String> = realorrug_roast::forbidden::check(&text)
        .into_iter()
        .map(|v| format!("forbidden: {v:?}"))
        .collect();
    why.extend(
        realorrug_roast::fidelity::check(&text, &post.authorised)
            .into_iter()
    ...
    for (n, post) in posts.iter().enumerate() {
        ...
        let text = match checked {
            ...
            Err(why) => {
                ...
                eprintln!(
                    "realorrug-analyst: {label} post {n} refused: {}",
                    why.join("; ")
                );
                break;
            }
        ...
        match result {
            ...
            Err(why) => {
                ...
                if matches!(why, Undeliverable::Unconfigured) {
                    ...
                    eprintln!(
                        "realorrug-analyst: {label} post {n} recorded, not published (dry run)"
                    );
                } else {
                    eprintln!("realorrug-analyst: {label} post {n} failed: {why}");
                }
                break;
            }
        }
...
mod tests {
    use super::*;
    use realorrug_contest::{Entry as ContestEntry, Metrics, Ranked, Ranking, Week};
    ...
    fn record(winner: bool) -> Record {
        ...
        Record::close(
            WEEK,
            ranking,
            &realorrug_contest::Rules::published(["radar"]),
        )
    }

    /// A board, best first, the way `hunter::tally` returns one.
    fn placing(summoner: &str, signals: u64) -> realorrug_contest::hunter::Placing {
        realorrug_contest::hunter::Placing {
