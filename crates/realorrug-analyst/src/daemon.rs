...
///
/// The threshold is [`realorrug_roast::baserates::STALE_AFTER_DAYS`], not a second
...
fn rates_in_use(
    loaded: Result<BaseRates, realorrug_roast::baserates::NotLoaded>,
    today: &str,
    ...
    match loaded {
        ...
        Ok(rates) => {
            let notice = format!(
                "realorrug-analyst: base rates measured on {}",
                rates.measured_on
            );
            (Some(rates), notice)
        }
        Err(e) => (
            None,
            format!(
                "realorrug-analyst: no base rates ({e}); replies will carry no population context"
            ),
        ),
    }
}
...
pub fn posture(has_credential: bool, can_post: bool, publishing: bool) -> &'static str {
    match (has_credential, can_post, publishing) {
        (false, _, _) => {
            "realorrug-analyst: no credential, so nothing is read and nothing is posted."
        }
        (true, _, false) => {
...
/// midnight.
///
/// That is not hypothetical. `deploy/realorrug-analyst.service` had exactly one
...
pub fn contest_writable_notice(contest_dir: &str) -> Option<String> {
    ...
    Some(format!(
        "realorrug-analyst: {contest_dir} CANNOT BE WRITTEN, so the week will not close and \
...
pub fn provider_notice(why: &realorrug_model::Selection) -> String {
    match why {
        ...
        _ => format!(
            "realorrug-analyst: a model provider is configured and UNUSABLE, so every reply is \
...
pub fn run() -> ! {
    ...
    // neither log.
    {
        ...
        eprintln!(
            "realorrug-analyst: gate restored — {} sent today, {} mints inside the dedupe window",
            gate.sent_today(),
    ...
    eprintln!(
        "realorrug-analyst: publisher={} source={} telegram={} dir={dir}",
        publisher.name(),
...
pub fn bio_notice(bio: Option<&crate::bio::Bio>) -> String {
    match bio {
        None => "realorrug-analyst: the bio is not written (REALORRUG_BIO_LEAD unset), so the \
            ...
            .to_owned(),
        Some(b) => format!(
            "realorrug-analyst: the bio IS written, at most hourly, as \"{}\" plus the week's \
...
fn bio_to_write(
    ...
    match crate::bio::check(&status, &state.authorised()) {
        ...
        Err(why) => {
            eprintln!(
                "realorrug-analyst: the bio was refused by its own checks and not written: {why}"
            );
            None
        }
    }
...
fn write_bio_if_changed(x: Option<&X>, bio: &crate::bio::Bio, spend: &mut Spend, paths: &Paths) {
    ...
    let Ok(reservation) = spend.authorize(Cost::Post, day_of(at)) else {
        eprintln!("realorrug-analyst: budget spent; the bio is not written");
        return;
    };
    match x.update_profile(&text) {
        Ok(()) => {
            ...
            if let Err(e) = std::fs::write(&path, BioMarker::render(at, &text)) {
                eprintln!("realorrug-analyst: the bio was written but not recorded: {e}");
            }
        }
        Err(e) => {
            spend.release(reservation);
...
pub fn build_notice(operators: usize) -> String {
    format!(
        "realorrug-analyst: build {}; operators: {operators} {}.",
        realorrug_types::build_sha_or_unknown(),
...
pub fn tick(
    ...
    let mut journal = match realorrug_journal::Journal::open(&paths.journal) {
        Ok(journal) => journal,
        Err(e) => {
            eprintln!(
                "realorrug-analyst: cannot open the journal at {}: {e}; not publishing this tick",
                paths.journal
            );
            return 0;
        }
    ...
    let Ok(read) = spend.authorize(Cost::MentionRead, today) else {
        eprintln!("realorrug-analyst: budget spent; not polling");
        return 0;
    };
    ...
    let mentions = match x.mentions(cursor.as_deref()) {
        ...
        Err(e) => {
            ...
            spend.release(read);
            eprintln!("realorrug-analyst: mentions poll failed: {e}");
            return 0;
        }
    };
    ...
    for mention in &mentions {
        ...
        if let Some(week) = crate::contest::try_claim(mention, &paths.contest_dir, at, |a| {
            ...
            eprintln!(
                "realorrug-analyst: {} -> claim recorded for week {}",
                mention.id, week.0
            );
        ...
        match outcome {
            Answered::Reply { entry, sheet, .. } => {
                ...
                let Ok(reply_cost) = spend.authorize(Cost::Reply, today) else {
                    ...
                    eprintln!(
                        "realorrug-analyst: reply budget spent; {} not answered, and \
                    ...
                    if let Err(e) = crate::contest::append_refusal(
                        ...
                        eprintln!("realorrug-analyst: cannot write {}: {e}", paths.refusals);
                    }
                    break;
                };
            ...
            other => {
                ...
                eprintln!("realorrug-analyst: {} -> {other:?}", mention.id);
                handled.push(&mention.id);
        ...
        && let Err(e) = poll::write_cursor(&paths.cursor, &next)
    {
        ...
        eprintln!("realorrug-analyst: cannot save the cursor: {e}");
    }
    if let Err(e) = spend.save() {
        eprintln!("realorrug-analyst: cannot save the ledger: {e}");
    }
    answered
}
...
mod tests {
    ...
    fn bio_record() -> realorrug_contest::Record {
        let entry = realorrug_contest::Entry {
            ...
            mint: "M".to_owned(),
            at: realorrug_contest::Week(2958).opens_at() + 10,
            metrics: realorrug_contest::Metrics::default(),
        };
        realorrug_contest::Record::close(
            realorrug_contest::Week(2958),
            realorrug_contest::Ranking {
    ...
    fn a_contest_directory_that_cannot_be_written_is_said_at_start_not_at_midnight() {
        ...
        // is what `deploy/realorrug-analyst.service` did until 2026-09-07, with
        ...
        let dir = std::env::temp_dir().join(format!("realorrug-contest-w{}", std::process::id()));
    ...
    fn a_fresh_snapshot_is_used_and_named() {
        ...
        let rates = realorrug_roast::BaseRates::parse(BASE_RATES).expect("the fixture parses");
    ...
    fn a_snapshot_past_the_threshold_is_dropped_and_says_why() {
        ...
        assert!(notice.contains("stale"), "{notice}");
        assert!(
            notice.contains(&realorrug_roast::baserates::STALE_AFTER_DAYS.to_string()),
    ...
    fn a_snapshot_that_would_not_load_is_absent_rather_than_fatal() {
        ...
        let (used, notice) = rates_in_use(
            Err(realorrug_roast::baserates::NotLoaded::Unreadable {
