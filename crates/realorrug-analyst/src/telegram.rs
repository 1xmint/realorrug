...
pub fn tick(
    ...
    let page = match bot.updates(cursor.as_deref()) {
        Ok(page) => page,
        Err(e) => {
            eprintln!("realorrug-analyst: telegram poll failed: {e}");
            return 0;
        }
    };
    ...
    for mention in &page.mentions {
        ...
        let reserved = provider.and_then(|_| {
            spend
                ...
                .inspect_err(|_| {
                    eprintln!(
                        "realorrug-analyst: model budget spent; telegram {} answered by the template",
                        mention.id
                    );
                })
                .ok()
        ...
        match outcome {
            ...
            other => eprintln!("realorrug-analyst: telegram {} -> {other:?}", mention.id),
        }
    }
    ...
    {
        eprintln!("realorrug-analyst: cannot save the telegram cursor: {e}");
    }
