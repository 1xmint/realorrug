...
//!
//! `realorrug-roast` writes a reply and cannot post it. This module posts and
...
use realorrug_journal::{Correlation, Journal, Outcome, Stage};
...
/// got — the same shape `realorrug_model::Provider` uses, and for the same reason:
...
pub fn publish(
    ...
    let intent = journal
        .record(
            ...
            realorrug_types::build_sha().map(ToOwned::to_owned),
            Vec::new(),
            None,
            None,
        )
