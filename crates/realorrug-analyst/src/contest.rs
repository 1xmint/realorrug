...
use realorrug_contest::hunter::{Placing, Sighting, tally};
use realorrug_contest::{
...
pub fn scan_ranking(
    ...
    ranked: &[realorrug_contest::Ranked],
    closes_at: u64,
    min_age_days: u32,
    ...
    for (i, r) in ranked.iter().enumerate() {
        ...
        if !meter.reserve(WORST_CASE_RESOURCES) {
            eprintln!(
                "realorrug-analyst: the day's budget will not cover scanning {}; \
        ...
        let engagers = match read(&r.entry.reply_id) {
            ...
            Err(e) => {
                ...
                meter.settle(0);
                eprintln!(
                    "realorrug-analyst: cannot scan {}: {e}; the week is scored on what was read",
                    r.entry.reply_id
                );
                break;
            }
...
struct BudgetMeter<'a> {
    ...
    held: Vec<realorrug_provider::Commitment>,
}
...
pub fn try_claim(
    ...
    contest_dir: &str,
    now: u64,
    owner_of: impl FnOnce(&realorrug_types::Address) -> Result<Option<String>, String>,
) -> Option<Week> {
    ...
    let Ok(parsed) = address.parse::<realorrug_types::Address>() else {
        return None;
    };
    let mut record = realorrug_contest::records_in(std::path::Path::new(contest_dir))
        .into_iter()
        .find(|r| {
    ...
    match owner_of(&parsed) {
        ...
        Ok(owner) => {
            eprintln!(
                "realorrug-analyst: week {} claim named {address}, owned by {}; not a wallet and not recorded",
                record.week.0,
        ...
        Err(e) => {
            eprintln!(
                "realorrug-analyst: week {} claim named {address} and its owner could not be read ({e}); not recorded, the winner can try again",
                record.week.0
            );
            return None;
        }
    ...
    record.claim = Some(realorrug_contest::Claim {
        address,
    ...
    if let Err(e) = write_atomically(&path, &text) {
        eprintln!("realorrug-analyst: cannot write the claim to {path}: {e}");
        return None;
    }
    Some(record.week)
}
...
fn scored_reads(
    ...
    let Some(x) = x else {
        eprintln!(
            "realorrug-analyst: week {} has {} entries and no X client to score them; not closing",
            week.0,
            entries.len()
        );
        return None;
    ...
    if !charge_post_reads(spend, ids.len(), day) {
        eprintln!(
            "realorrug-analyst: the day's budget will not cover reading {} replies' metrics; \
             not closing week {}",
            ids.len(),
            week.0
        );
    ...
    let metrics = match x.metrics(&ids) {
        Ok(m) => m,
        Err(e) => {
            eprintln!("realorrug-analyst: cannot read the week's metrics: {e}; not closing");
            return None;
        }
    };
    if !charge_user_reads(spend, summoners.len(), day) {
        eprintln!(
            "realorrug-analyst: the day's budget will not cover reading {} entrants' accounts; \
             not closing week {}",
            summoners.len(),
            week.0
        );
    ...
    let accounts = match x.accounts(&summoners) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("realorrug-analyst: cannot read the entrants' accounts: {e}; not closing");
            return None;
        }
    };
...
pub fn close_if_due(
    ...
    let previous = realorrug_contest::records_in(std::path::Path::new(&paths.contest_dir));
...
/// [`realorrug_onchain::RpcClient::owner_of`] returned.
///
...
///
/// The payout has its own copy of this refusal (`realorrug_payout`), and that is
...
pub fn prompt_due(contest_dir: &str, now: u64) -> Option<Record> {
    realorrug_contest::records_in(std::path::Path::new(contest_dir))
        .into_iter()
...
mod tests {
    ...
    fn a_wallet(_: &realorrug_types::Address) -> Result<Option<String>, String> {
        Ok(None)
    }
    ...
    fn ranked_with(reply_id: &str, reposts: u64, quotes: u64) -> realorrug_contest::Ranked {
        let metrics = Metrics {
            reposts,
            quotes,
            likes: 0,
    ...
    fn unfunded() -> crate::spend::Spend {
        crate::spend::Spend::open(
            realorrug_provider::Budget::CLOSED,
            crate::spend::Prices {
                mention_read: realorrug_types::MicroUsd(1_000),
                post_read: realorrug_types::MicroUsd(5_000),
                reply: realorrug_types::MicroUsd(10_000),
                post: realorrug_types::MicroUsd(15_000),
    ...
    fn the_week_closes_from_its_inputs_and_an_unscored_reply_is_excluded_not_zero() {
        ...
        last_week.winner = Some(realorrug_contest::Winner {
    ...
    fn voiding_a_week_records_the_reason_and_refuses_to_be_silent() {
        ...
        let rules = Rules::published(["radar"]);
        let record = Record::close(WEEK, realorrug_contest::Ranking::default(), &rules);
        ...
        let reread = realorrug_contest::records_in(std::path::Path::new(&path));
        assert_eq!(
    ...
    fn a_week_cannot_be_voided_twice_or_after_it_was_paid() {
        ...
        write_record(
            &path,
            &Record::close(WEEK, realorrug_contest::Ranking::default(), &rules),
        )
        .expect("write");
    ...
    fn a_claim_naming_a_coins_mint_is_refused_rather_than_recorded() {
        ...
        let rules = Rules::published(["radar"]);
        let mut record = Record::close(WEEK, realorrug_contest::Ranking::default(), &rules);
        record.winner = Some(realorrug_contest::Winner {
            summoner: "alice".to_owned(),
        ...
        assert!(
            realorrug_contest::records_in(std::path::Path::new(&dir))[0]
                .claim
                .is_none(),
    ...
    fn a_reply_to_the_claim_prompt_is_a_claim_and_everything_else_is_a_summons() {
        ...
        write_atomically(&record_path(&dir, WEEK), &record.to_json().expect("json"))
            .expect("write");
        let address = realorrug_types::Address::new([5u8; 32]).to_string();
        ...
        let saved = realorrug_contest::records_in(std::path::Path::new(&dir));
        ...
        let other = realorrug_types::Address::new([6u8; 32]).to_string();
        assert_eq!(
            try_claim(
    ...
    fn a_week_whose_prompt_never_posted_accepts_no_claim_at_all() {
        ...
        write_atomically(&record_path(&dir, WEEK), &record.to_json().expect("json"))
            .expect("write");
        let address = realorrug_types::Address::new([7u8; 32]).to_string();
        let inside = WEEK.closes_at() + 3_600;
    ...
    fn the_budget_meter_charges_what_the_read_billed_and_gives_back_the_rest() {
        ...
        assert_eq!(
            spend.spent_today(),
            realorrug_types::MicroUsd(40_000),
    ...
    fn a_scan_that_read_nothing_costs_nothing() {
        ...
        assert_eq!(spend.spent_today(), realorrug_types::MicroUsd(0));
    }
    ...
    fn charging_for_a_lookup_stops_at_the_budget_rather_than_overrunning_it() {
        ...
        assert_eq!(spend.spent_today(), realorrug_types::MicroUsd(15_000));

        let mut broke = unfunded();
        assert!(!charge_post_reads(&mut broke, 1, 1));
        assert!(!charge_user_reads(&mut broke, 1, 1));
        assert_eq!(broke.spent_today(), realorrug_types::MicroUsd(0));
    }
}
