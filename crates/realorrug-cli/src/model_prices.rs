...
//! `realorrug model-prices` — what to paste into `analyst.env`, from the catalog.
//!
//! # Why a command and not a lookup in the daemon
//!
//! The daemon never reads the catalog. [`realorrug_model::catalog`]'s module note
...
/// The usage line.
const USAGE: &str = "usage: realorrug model-prices <model>            the lines to paste
...
mod tests {
    ...
    fn no_arguments_prints_usage_without_reaching_the_network() {
        ...
        assert!(why.starts_with("usage: realorrug model-prices"), "{why}");
    }
