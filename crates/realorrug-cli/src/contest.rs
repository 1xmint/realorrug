...
//! `realorrug contest`: the payout's manual fallback, through the same check.
//!
//! Design 0007 C5. Two subcommands, and neither holds a key:
//!
//! - `pay --week N --creator <address> --rpc <url> [--contest-dir <dir>]`
//!   plans the week's payment exactly as `realorrug-payout` would and prints the
...
use realorrug_payout::{Chain, Rpc, plan, record_payout};
use realorrug_pumpfun::pda;
...
reason, published verbatim: {}",
    ...
    match sub {
        Some("pay") => {
            ...
            let record = realorrug_payout::read_record(&contest_dir, week).map_err(|e| e.to_string())?;
            ...
            let floor = realorrug_payout::floor_from(&|k| std::env::var(k).ok());
            let planned = plan(&record, &creator, lamports, &blockhash, floor)
                .map_err(|e| e.to_string())?;
            println!(
                "week {}: pay {} lamports from {} to {}\nvault {} holds {} lamports\nunsigned transaction, base64 (sign with the creator key and send; then `realorrug contest record-payout --signature <sig>`):\n{}",
                week.0,
                planned.lamports,
                planned.creator,
                planned.recipient,
        ...
        _ => Err(
            "realorrug contest <pay --dry-run | record-payout --signature <sig> | void --reason <words>> --week <n> --creator <address> --rpc <url>"
                .to_owned(),
        ),
    }
}
...
mod tests {
    ...
    fn pay_refuses_to_do_anything_without_dry_run_before_it_touches_a_chain() {
        ...
        let creator = realorrug_types::Address::new([1u8; 32]).to_string();
        let out = run(&args(&[
            "contest",
            "pay",
            "--week",
