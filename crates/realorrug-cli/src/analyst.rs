...
//! `realorrug analyst` — the whole summoned-reply loop, over mentions from a file.
//!
//! The caller `realorrug-analyst` was built for, and the way the loop is exercised
...
//!
//! **It cannot post.** The only [`Publisher`](realorrug_analyst::Publisher) wired
...
//! behind it is not written — see `realorrug_analyst::publish` for the two billing
...
///
/// Hand-parsed from a `Value` rather than derived, because `realorrug-cli` does not
...
///
/// The type is [`realorrug_analyst::Mention`], the same one the X client produces.
...
///
/// The pipeline itself is [`realorrug_analyst::answer`], shared with the daemon:
...
=== {} from @{}",
    ...
    let mut journal = realorrug_journal::Journal::open(&journal_path)
        .map_err(|e| format!("could not open the journal at {journal_path}: {e}"))?;
    let written = realorrug_analyst::publish::publish(
        &realorrug_analyst::DryRun,
        log_path,
        &mut journal,
        entry,
    )
