...
//! `realorrug audit` — reading the journal back.
//!
...
use realorrug_journal::{Event, Journal, Outcome, Stage, Verified};
...
fn export(path: &str, week: &str) -> Result<(), String> {
    ...
    let bundle = serde_json::json!({
        ...
        "how_to_read": "Each event's `previous` names the one before it. Run \
                        `realorrug audit verify --journal <path>` against the source \
...
mod tests {
    use super::*;
    use realorrug_journal::Correlation;
