...
//! [`realorrug_provider`] and [`radar_risk`], and for the same reason. A component
...
#[derive(Debug)]
pub struct Agent {
    meter: realorrug_provider::Meter,
    allowlist: Allowlist,
...
impl Agent {
    ...
    pub fn unconfigured(day: u64) -> Self {
        Self {
            meter: realorrug_provider::Meter::new(
                Budget {
    ...
    pub fn new(config: Config, day: u64) -> Self {
        Self {
            meter: realorrug_provider::Meter::new(config.budget, day),
    ...
    pub fn restore(config: Config, ledger: &Ledger, day: u64) -> Self {
        Self {
            meter: realorrug_provider::Meter::restore(config.budget, ledger, day),
            allowlist: config.allowlist,
            budget: config.budget,
            configured: true,
        }
    ...
    pub fn begin(
        ...
        estimate: MicroUsd,
        day: u64,
    ) -> Result<realorrug_provider::Commitment, Unavailable> {
        if !self.configured {
    ...
    pub fn settle(&mut self, commitment: realorrug_provider::Commitment, actual: MicroUsd) {
    ...
    pub fn abandon(&mut self, commitment: realorrug_provider::Commitment) {
