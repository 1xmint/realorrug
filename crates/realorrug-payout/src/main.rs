...
use realorrug_payout::{Outcome, PayError, Rpc, load_key, pay};
...
fn main() -> ExitCode {
    ...
    let Some(key_path) = env("RADAR_PAYOUT_KEY") else {
        eprintln!("realorrug-payout: RADAR_PAYOUT_KEY is not set, so nothing is paid.");
        return ExitCode::FAILURE;
    };
    let Some(rpc_url) = env("RADAR_RPC_URL") else {
        eprintln!("realorrug-payout: RADAR_RPC_URL is not set, so nothing is paid.");
        return ExitCode::FAILURE;
    };
    ...
    eprintln!("{}", realorrug_payout::floor_notice(floor));
...
mod tests {
    use super::*;
    use realorrug_contest::{Claim, Payout, Ranking};
