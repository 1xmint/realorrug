...
use realorrug_pumpfun::fees::{FeeConfig, Fees, global_fees};
...
fn a_market_cap_below_every_tier_has_no_fee_rather_than_a_free_trade() {
    ...
    let config = FeeConfig {
        flat: Fees::default(),
        tiers: vec![realorrug_pumpfun::fees::Tier {
...
fn synthetic(tiers: &[(u128, u64, u64, u64)]) -> Vec<u8> {
    let mut out = Vec::new();
    out.extend_from_slice(&realorrug_pumpfun::fees::FEE_CONFIG_DISCRIMINATOR);
    out.push(255); // bump
