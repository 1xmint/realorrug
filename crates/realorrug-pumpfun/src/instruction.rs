...
//! come to disagree with `realorrug-decode` about the same program, and the
//! disagreement would be invisible until a transaction was rejected. So the
//! bytes are looked up in `realorrug_decode::pumpfun::KNOWN` by the instruction's
//! own name, and a lookup that fails is a refusal rather than a fallback.

use realorrug_decode::pumpfun::{Instruction, KNOWN};
...
pub fn collect_creator_fee(creator: &Address) -> Option<crate::transaction::Instruction> {
    ...
    Some(crate::transaction::Instruction {
        program_id: realorrug_decode::pumpfun::PROGRAM_ID,
        accounts: vec![
            ...
            AccountMeta::readonly(realorrug_decode::pumpfun::PROGRAM_ID),
        ],
...
mod tests {
    ...
    fn collect_creator_fee_names_the_idls_five_accounts_in_order_and_carries_only_its_discriminator()
     {
        ...
        assert_eq!(ix.program_id, realorrug_decode::pumpfun::PROGRAM_ID);
        assert_eq!(
            ix.data,
        ...
        assert_eq!(
            ...
            vec![
                ...
                (realorrug_decode::pumpfun::PROGRAM_ID, false, false),
            ]
        );
        assert_eq!(
    ...
    /// `realorrug-decode/tests/fixtures/pumpfun_accounts.json`.
