...
use realorrug_onchain::budget::{Budget, Count, Exhausted};
use realorrug_onchain::launch::{assemble, recipients_in};
use realorrug_onchain::rpc::{RawInstruction, TokenBalance, Transaction, parse_transaction};
...
fn a_creator_who_named_their_token_an_instruction_is_still_only_data() {
    ...
    assert_eq!(
        assembled.metadata.trust(),
        realorrug_types::Trust::Untrusted
    );
