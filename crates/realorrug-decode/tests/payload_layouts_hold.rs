...
use realorrug_decode::pumpfun::Instruction;
use realorrug_decode::{ArgError, Decoded, Launch, Layout, Program, Side, Trade, decode, pumpfun};
...
fn pumpfun_trade(data: &[u8]) -> Option<Result<Trade, ArgError>> {
    let Decoded::Known(realorrug_decode::Instruction::PumpFun(ix)) = decode(Program::PumpFun, data)
    else {
        return None;
    };
...
fn instruction_named(name: &str) -> Instruction {
    let Some((ix, _, _)) = realorrug_decode::pumpfun::KNOWN
        .iter()
        .find(|(_, _, n)| *n == name)
    else {
...
fn launch_metadata_decodes_as_creator_supplied_text() {
    ...
    for name in ["create_v2", "create"] {
        ...
        for s in &blob.samples {
            ...
            assert_ne!(
                l.creator,
                realorrug_types::Address::new([0u8; 32]),
