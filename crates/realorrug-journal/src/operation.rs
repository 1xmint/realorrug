...
//!
//! This is the gap `realorrug-analyst`'s `Publisher` trait names and cannot express:
...
//! [`Portfolio`]: realorrug_types::Portfolio
...
impl OperationLog {
    ...
    pub fn open(path: impl Into<std::path::PathBuf>) -> Result<Self, OperationError> {
        ...
        Ok(Self {
            journal,
            operations,
            build: realorrug_types::build_sha().map(str::to_owned),
        })
    }
    ...
    ///
    /// [`Portfolio::reserve`]: realorrug_types::Portfolio::reserve
    ...
    fn close(
        ...
        if let (Some(claim), Settlement::PartiallyFilled(filled)) = (claim, settlement) {
            let outstanding = portfolio
                .reservations()
                .find(|r| r.id() == claim)
                .map(realorrug_types::Reservation::outstanding);
...
mod tests {
    ...
    use realorrug_types::{Decimals, Settlement, TokenQuantity};
