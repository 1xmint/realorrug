...
fn the_operation_lines_are_a_chain_like_every_other_line() {
    ...
    let journal = realorrug_journal::Journal::open(&path).expect("open");
    assert_eq!(
        journal.verify().expect("verify"),
        realorrug_journal::Verified::Intact { events: 3 }
    );
...
fn an_established_failure_is_recorded_as_a_failure() {
    ...
    let events = realorrug_journal::Journal::open(&path)
        .expect("open")
        .events()
        .expect("events");
    let last = events.last().expect("a line");
    assert_eq!(last.outcome, realorrug_journal::Outcome::Failed);
...
fn an_event_about_nothing_operational_is_written_exactly_as_it_always_was() {
    ...
    let mut journal = realorrug_journal::Journal::open(&path).expect("open");
    journal
        .record(
            realorrug_journal::Stage::Received,
            realorrug_journal::Outcome::Ok,
            1,
            about(),
            None,
            Vec::new(),
    ...
    assert_eq!(
        journal.verify().expect("verify"),
        realorrug_journal::Verified::Intact { events: 1 }
    );
}
