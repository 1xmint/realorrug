...
use realorrug_journal::{Correlation, Journal, Outcome, Stage, Verified};
...
fn a_relinked_chain_is_still_caught_by_the_event_after_it() {
    ...
    let text = std::fs::read_to_string(&path).expect("read");
    let mut lines: Vec<String> = text.lines().map(ToOwned::to_owned).collect();
    let mut altered: realorrug_journal::Event =
...
fn an_oversized_diagnostic_is_refused_rather_than_truncated() {
    ...
    assert!(matches!(
        refused,
        Err(realorrug_journal::JournalError::RedactedTooLong { length: 600 })
    ));
}
...
fn the_diagnostic_bound_is_walked_from_both_sides() {
    ...
    let at_the_bound = journal.record(
        ...
        Vec::new(),
        None,
        Some("x".repeat(realorrug_journal::MAX_REDACTED)),
    );
    assert!(
        at_the_bound.is_ok(),
    ...
    let one_over = journal.record(
        ...
        Vec::new(),
        None,
        Some("x".repeat(realorrug_journal::MAX_REDACTED + 1)),
    );
    assert!(matches!(
        one_over,
        Err(realorrug_journal::JournalError::RedactedTooLong { .. })
    ));
}
