...
///
/// This is what `realorrug-onchain` actually produces for a mint it cannot reach:
...
fn index_with(
    creator: &str,
    record: realorrug_roast::creator::Record,
...
fn a_creators_record_is_what_makes_one_reply_differ_from_another() {
    ...
    let index = index_with(
        &creator,
        realorrug_roast::creator::Record {
            launches: 47,
            measured: 41,
            organic: 0,
            instant: 2,
    ...
    // fact nobody reads.
    let reply = realorrug_roast::verdict::template(&sheet);
    assert!(
        reply.contains("47"),
...
fn a_creator_with_no_record_is_said_to_have_none_rather_than_omitted() {
    ...
    let dossier = dossier_named("A Token", "TKN");
    let index = index_with("somebody-else", realorrug_roast::creator::Record::default());
