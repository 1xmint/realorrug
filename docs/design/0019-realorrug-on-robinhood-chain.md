...
launched.
**§5.3 amended by [ADR 0024](../adr/0024-the-bot-stands-alone.md)** (2026-09-13):
...
| 3 | 100% of the creator fee is the weekly prize | **survives, and since plan 0001 step 6c the code does too.** [`realorrug-payout`](../../crates/realorrug-payout/src/lib.rs) claims the creator's balance from the Pons fee escrow and transfers exactly what the escrow paid, signed through Turnkey, with gas from a float the operator funds ([ADR 0025](../adr/0025-the-robinhood-payout-signs-through-turnkey.md)). [`realorrug-contest`](../../crates/realorrug-contest/src/ledger.rs)'s ledger now holds wei, with the Solana arm kept read-only. **Correction, kept:** during the discussion I called the contest crate chain-agnostic. The rule was; the ledger's money types were not, until 6c |
...
| 5 | never states the token's price | **holds, with one change.** `REALORRUG_SELF_MINT` is parsed as a Solana address ([`daemon.rs`](../../crates/realorrug-analyst/src/daemon.rs)); it has to accept the token's EVM address, and a bridged Solana version's mint as well |
...
| crate | imported by |
|---|---|
| `realorrug-analyst` | `radar-serve`, `radar-cli`, `radar-backfill` |
| `realorrug-roast` | `radar-serve`, `radar-cli`, `radar-research` |
| `realorrug-contest` | `radar-serve`, `radar-cli`, `realorrug-payout`, `realorrug-analyst` |
| `realorrug-payout` | `radar-cli` |
...
- `radar-research` imports `realorrug_roast::creator` and `realorrug_roast::BaseRates`:
...
|---|---|
| `realorrug-analyst`, `realorrug-roast` (minus the two research types), `realorrug-contest`, the public receipt/leaderboard/pool routes, a new EVM payout | `realorrug-types`, `realorrug-onchain`, `realorrug-model`, `realorrug-agent`, `realorrug-provider`, `realorrug-journal`, `realorrug-decode` |
| | `realorrug_roast::creator` and `BaseRates` move **down** into a shared Radar crate first, so `radar-research` stops importing the bot |

- **Preserve history.** Move with `git filter-repo` on the moved paths, not a copy.
- **`realorrug-payout` is not moved; it is replaced.** Its pump.fun path has no
