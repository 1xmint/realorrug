...
| 3 | 100% of the creator fee is the weekly prize | **survives in principle, and the code does not.** [`realorrug-payout`](../../crates/realorrug-payout/src/lib.rs) is pump.fun's `collect_creator_fee` plus a SOL transfer. [`realorrug-contest`](../../crates/realorrug-contest/src/ledger.rs)'s rule has no chain in it, but its ledger does: the claim is "The Solana address, as text" and amounts are `lamports`. **Correction:** during the discussion I called the contest crate chain-agnostic. The rule is; the ledger's money types are not |
