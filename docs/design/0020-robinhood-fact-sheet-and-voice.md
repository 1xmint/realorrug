...
| correlated selling (S7, research 0052 §3.1): how many sellers inside one 50-block window are linked by how they bought, what share of supply they sold, over how many seconds | every `CurveBuy`/`CurveSell` log on the curve from the launch block to the read point, then the timestamps of the cluster's first and last sell block (`wallets.rs` `correlated_selling`). Pre-graduation only: after graduation the trades move to a pool whose address research 0044 has not found. A refused read, or a curve with more trades than one read returns, leaves the fact unread, never zero (AGENTS.md §3 rule 8), and names it as a coverage gap that cannot lower the level to `CantTell` | 1 `eth_getLogs` + up to 2 `eth_getBlockByNumber` (none when the cluster sold in one block) | 20 + 20 each | 60 + 80 each | optional |
...
| `LaunchBlockBundle` | launch-block distinct recipient count lands in the strongest measured band | distinct non-zero-balance recipients of `Transfer` logs in the launch block, looked up in a measured snapshot's bands (the same `baserates.rs::band_for` shape, re-derived for Pons v2 — **not yet measured**, §8) | **read from the snapshot, never a constant** — research 0042's first lesson, quoted there: *"Six is a tool's default, not a law. The number will move when whoever is running this changes their configuration, and the detector will go quiet without saying so"* (0008, quoted in 0042), which came true in 0024's re-measurement | a launch people were waiting for — no chain fact distinguishes a bundle bought by insiders from a bundle bought by fans; only the *rate* at which each recipient count precedes an outcome, measured, does — **factor table (M-D-weights-s2-s5-s13, research 0052 §3 row S2, `LaunchBlockInStrongestBand`):** wired (`sheet.rs::launch_block_band_factors`, `push_window_buyer_factors`, `push_band`) — `+1,000` for checked same-window buyers holding `>= 1,000` bps together (raw, `linked_holdings_bps`), `+800` for `>= 3` fresh checked buyers, `+500` for checked buy sizes within 10% of each other, `-500` for every launch-window buyer being declared-exempt (only when `funding.checked` is the *full* buyer list, never the cost-limited sample), `-300` for a band measured on `< 200` launches (derived as `band.fires_on * BaseRates::launches`, since `Band` itself carries no raw count). **Still dormant on a real Robinhood dossier today**, the same as S5's `HolderConcentration` below: this signal only fires on a Solana `dossier.launch`, while the four buyer-derived facts it reads come from `dossier.funding`/`dossier.powers`/`ChainLaunch::supply`, which are Robinhood-only. The two conditions do not co-occur on real data yet, so these factors are exercised only by a sheet built directly in a test |
...
| `HolderConcentration` | largest non-curve holder's share | `Transfer`-log sum (research 0040 §4), largest balance divided by circulating (total minus curve), on Robinhood; `Kind::TokenOwnership`'s sampled largest accounts, curve and PumpSwap pool excluded, on Solana | **pushed at `>= 1,000` bps** (task 9-23-0007), reusing research 0052 §3 row S5's own lower factor raise as the signal's own floor since no separate distribution of real holder-concentration outcomes has been gathered to measure a threshold of its own (open question, §8 still stands for *that* number) | a vesting contract, a bridge, an exchange — none of which this design can currently tell apart from a whale, because no label list for Robinhood-chain contracts exists yet (the EVM equivalent of research 0042's "exchange hot wallet, bridge contract, known deployer" label-list idea, itself flagged there as needed and not yet built for Solana either) — **factor table (M-D-weights-s2-s5-s13, research 0052 §3 row S5):** the two bps-threshold raises are wired in `sheet.rs`'s `holder_concentration_factors` off the existing `Kind::LargestHolderShare` fact, standing in for "largest non-infrastructure" per that row's own "unresolved large balance keeps full weight" language; the top-10 raise and the role-`Proof`-gated lower are not wired because neither fact reaches the sheet. The signal itself is now pushed by `FactSheet::build` on both chains; the Solana `Kind::TokenOwnership` path is not yet read by `holder_concentration_factors`, so the score-shadow factor weighting still runs only against Robinhood facts and directly-constructed test sheets, even though the signal itself fires on both |
| `CorrelatedSelling` | wallets that look linked sold together | `CurveSell` logs in one 50-block window, each seller linked to the window's first seller by how they bought (buys within 30 blocks of each other with sizes within 10%: link confidence ≥ 4,000 bps, research 0052 §3.2; same-block buying alone does not link). Only the sales inside the window count toward its size, tokens and spread. Link evidence is buy-side only, so the sell cannot prove its own link | at least **2** linked sellers fires it; one seller is not correlated with anything and says nothing. The 3-wallet line is a factor, not the threshold | friends who bought together at launch and took profit at the same pump; the spread-over-an-hour factor is the only hedge against that today |
| `OwnerCanStillMintOrPause` | **on Pons v2, not the bytecode scan this row originally named — [ADR 0035](../adr/0035-s13-owner-powers-live-is-the-pons-v2-reads-not-a-bytecode-scan.md) settled the conflict this row and research 0052 §3 both flagged.** The bytecode/ABI version below stays parked, unbuilt, until research 0044 answers the stock-Ownable false-positive twin; it is not what this variant fires on for Pons v2 today | *(parked, research 0044)* deployed bytecode scan, or a verified-source ABI read for owner-only mint/pause/blacklist selectors and ownership renouncement. *(live, Pons v2, ADR 0035)* creator tax, `pendingCreatorFeeRecipient` and classified snipe-tax exemptions, read by `robinhood.rs`'s `powers_facts` (M-D-0004) and projected onto the sheet by `sheet.rs`'s `push_powers` (M-D-S13-owner-powers) | **blocked on research 0044** for the parked bytecode version; the Pons v2 version is wired and measured (ADR 0035, research 0052 §3.1's S13 row) | a stock template with an owner nobody uses — most ERC-20 templates ship an `Ownable` ancestor whether or not the deployer ever calls it — this remains the parked version's twin. The live Pons v2 version's own twin is `sheet.rs::twin_for`'s wording for this signal — **factor table (M-D-S13-owner-powers, research 0052 §3 row S13, ADR 0035):** `+700` creator tax `>= 500` bps, `+500` `pendingCreatorFeeRecipient` non-zero, `+600` per `Undeclared`-classified exempt address capped at `+1,200`, `-300` creator tax `== 0`, all wired in `sheet.rs`'s `factors` |
...
| `CorrelatedSelling` | ≥ 3 linked wallets sold within 50 blocks | +800 bps | Measured | `Kind::CorrelatedSellWallets`, from `ChainLaunch::correlated_selling` |
| `CorrelatedSelling` | they sold ≥ 1,000 bps of supply | +600 bps | Measured | `Kind::CorrelatedSellVolumeBps`, from the same read and `ChainLaunch::supply` |
| `CorrelatedSelling` | their sells spread over > 3,600 s (cannot fire at today's ~0.1 s blocks, where 50 blocks is about 5 s; kept for calibration, M-D-0006, to re-size the window) | −300 bps | Measured | `Kind::CorrelatedSellSpreadSeconds`, from the first and last sell block's timestamps |
...
- `LaunchBlockInStrongestBand`'s fresh-wallet, linked-wallet, exemption and
  thin-sample factors are now wired (see the signal table above), but remain
  dormant on real data: the signal fires only on Solana, and the facts it
  reads come only from Robinhood dossiers.
...
- **Research 0044 (bytecode checks — owner-only mint/pause/blacklist,
  ownership renouncement), in flight, and now parked behind [ADR
  0035](../adr/0035-s13-owner-powers-live-is-the-pons-v2-reads-not-a-bytecode-scan.md).**
  `OwnerCanStillMintOrPause` (§3) fires on Pons v2 today off creator tax,
  `pendingCreatorFeeRecipient` and snipe-tax exemptions instead of a bytecode
  scan — ADR 0035's resolution of the conflict this section and research
  0052 §3 both once flagged. The bytecode-scan version this note originally
  described stays a real, separate thing worth building for a chain without
  Pons v2's admin-surface research, and 0044 still owes it: the actual
  ...
  it no longer owes anything to this variant on Pons v2.
