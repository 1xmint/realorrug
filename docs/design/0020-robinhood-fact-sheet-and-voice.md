...
| `CorrelatedSelling` | wallets that look linked sold together | `CurveSell` logs in one 50-block window, each seller linked to the window's earliest seller by how they bought (same block, or the same 30-block window with sizes within 10%: link confidence ≥ 4,000 bps, research 0052 §3.2). Link evidence is buy-side only, so the sell cannot prove its own link | at least **2** linked sellers fires it; one seller is not correlated with anything and says nothing. The 3-wallet line is a factor, not the threshold | friends who bought together at launch and took profit at the same pump; the spread-over-an-hour factor is the only hedge against that today |
...
| `CorrelatedSelling` | their sells spread over > 3,600 s | −300 bps | Measured | `Kind::CorrelatedSellSpreadSeconds`, from the first and last sell block's timestamps |
